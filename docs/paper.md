...
1. **Dataset 1**: Focuses on Cluster Load Allocation (CLA) energy estimation.
   - **Ground-Truth & Consumer Unit Features ($X_R$):** `gt_scenario_id`, `gt_feeder_id`, `gt_consumer_unit_id`, `consumer_unit_source` (bus and line connected to), `consumer_unit_loads`, `latent_consumer_unit_source`, `latent_consumer_unit_loads`, `measured_energy_kwh` (populated for metered units, empty for unmetered), `gt_total_consumer_energy_kwh`, `gt_technical_loss_kwh` (transformer loss + line loss), `gt_non_technical_loss_kwh`.
   - **Estimator Predictions:** `cla_estimates` (populated for unmetered units, empty for metered), `time_adjusted_cla_estimates` (populated for unmetered units, empty for metered).
...
2. **Dataset 2**: Evaluates what type of co-events are observable across load switch pairs (`load_load`) and mixed load switch and fault pairs (`load_fault`).
   - **Ground-Truth Target Variables ($X_R$):** `gt_scenario_id`, `gt_transformer_id`, `gt_feeder_id`, `gt_boundary_unit_id`, `gt_pair_category`, `load_source`, `fault_info`, `gt_event_1_class`, `gt_event_1_type`, `gt_event_2_class`, `gt_event_2_type`.
...
3. **Dataset 3**: Evaluates how residual magnitude in pair varies with time shift operation ($t_{\mathrm{offset}} = 0.0\,\mathrm{s}$ vs $t_{\mathrm{offset}} > 0.0\,\mathrm{s}$) across load switch pairs and mixed load-fault pairs.
   - **Ground-Truth Target Variables ($X_R$):** `gt_scenario_id`, `gt_transformer_id`, `gt_feeder_id`, `gt_boundary_unit_id`, `gt_pair_category`, `load_source`, `fault_info`, `gt_event_1_class`, `gt_event_1_type`, `gt_event_2_class`, `gt_event_2_type`, `gt_time_offset_s`.
...
4. **Dataset 4**: Evaluates how transformer specification affects the observability of load switch pairs and mixed load-fault pairs. Featuring varying transformer specifications whose physical, sequence, and capability parameters are explicitly specified in `docs/specs/lv1/lv_transformer.md`, `docs/specs/lv2/lv_transformer.md`, and `docs/specs/lv3/lv_transformer.md`.
   - **Ground-Truth Target Variables ($X_R$):** `gt_scenario_id`, `gt_lv_network_id`, `gt_transformer_id`, `gt_transformer_spec_id`, `gt_feeder_id`, `gt_boundary_unit_id`, `gt_pair_category`, `load_source`, `fault_info`, `gt_event_1_class`, `gt_event_1_type`, `gt_event_2_class`, `gt_event_2_type`.
