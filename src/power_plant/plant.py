...
from src.transient.atp_case_builder import NetworkEquivalent, TestBranchModel
...
def extract_network_equivalent(dss_instance, feeder_idx: int = 1) -> NetworkEquivalent:
    """
    Extracts the reduced 3x3 base network equivalent at the LV boundary of the target transformer
    from the pre-event OpenDSS operating state (without transient test loads or faults).
    """
    sec_bus = f"feeder{feeder_idx}_sec"
    if not dss_instance.Circuit.SetActiveBus(sec_bus):
        raise ValueError(f"Could not set active bus '{sec_bus}' in OpenDSS for network reduction")

    tx_elem = f"Transformer.trans{feeder_idx}"
    if not dss_instance.Circuit.SetActiveElement(tx_elem):
        raise ValueError(f"Could not set active element '{tx_elem}' in OpenDSS for network reduction")

    powers = dss_instance.CktElement.Powers()
    # Secondary winding powers (P_A, Q_A, P_B, Q_B, P_C, Q_C)
    ...
    v_rms = [float(v_mag_angle[0]), float(v_mag_angle[2]), float(v_mag_angle[4])]
    v_ang = [float(v_mag_angle[1]), float(v_mag_angle[3]), float(v_mag_angle[5])]
    ...
    for idx in range(3):
        ...
        if s_sq > 0:
            ...
            r = 100.0
            x = 10.0
        zth_diag.append(r + 1j * x)

    zth_matrix = np.diag(zth_diag)

    vth = (
        v_rms[0] * np.exp(1j * np.radians(v_ang[0])),
        v_rms[1] * np.exp(1j * np.radians(v_ang[1])),
        v_rms[2] * np.exp(1j * np.radians(v_ang[2]))
    )

    return NetworkEquivalent(
        name=f"base_lv_network_feeder{feeder_idx}",
        side="lv",
        vth_v=vth,
        zth_ohm=zth_matrix,
        reference_bus=sec_bus
    )


def build_test_branches(event: Any, frequency_hz: float = 50.0) -> List[TestBranchModel]:
    """
    Constructs explicit TestBranchModel instances from transient switching or fault events.
    """
    test_branches = []

    events_list = []
    if hasattr(event, "event_1") and hasattr(event, "event_2") and event.event_1 is not None and event.event_2 is not None:
        events_list = [event.event_1, event.event_2]
    else:
        events_list = [event]

    for idx, ev in enumerate(events_list):
        ev_class = getattr(ev, "event_class", getattr(event, "event_class", None))
        if ev_class in ["load_switch", "equipment_switch", "co_event"]:
            eq_type = getattr(ev, "equipment_type", None)
            if not eq_type:
                continue
            ...
            for key in ["l_armature", "l_coil", "l_ac_filter", "l_filter"]:
                if key in eq_model.atp_params:
                    l_h_val = float(eq_model.atp_params[key])
                    break
            ...
            if l_h_val == 0.0:
                for key in ["x_stator", "x_rotor"]:
                    if key in eq_model.atp_params:
                        ...
                        l_h_val = x_val / (2.0 * np.pi * frequency_hz)
                        break

            if r_val is not None:
                for ph in ["A", "B", "C"]:
                    test_branches.append(TestBranchModel(
                        name=f"test_eq_{idx}_{ph}",
                        from_bus=f"LV_{ph}",
                        to_bus=f"TST_{idx}_{ph}",
                        r_ohm=r_val,
                        l_h=l_h_val,
                        c_f=0.0,
